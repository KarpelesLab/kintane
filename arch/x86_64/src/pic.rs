...
//! x86-64 image must drive this on a machine that has nothing better and an APIC on
//! one that does. So this is a driver behind `dyn IrqChip` rather than an inherent
//! part of `X86_64`, exactly as `docs/portability.md` describes — the architecture
//! layer is static and generic, the device layer is dynamic.
//!
...
impl IrqChip for Pic8259 {
    ...
    unsafe fn init(&self) {
        ...
        unsafe {
            ...
            // controller that delivers an interrupt before its handler exists.
