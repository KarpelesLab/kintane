...
impl Serial {
    ...
    pub unsafe fn init(&self) {
        ...
        // ports, which exist on every PC-compatible machine and on QEMU's q35.
        unsafe {
...
pub(crate) fn write_hex(c: &dyn EarlyConsole, v: u64, digits: usize) {
    let mut buf = [0u8; 18];
    buf[0] = b'0';
    buf[1] = b'x';
    let n = if digits > 16 { 16 } else { digits };
    for i in 0..n {
        let shift = (n - 1 - i) * 4;
