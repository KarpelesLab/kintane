...
/// then the hook. On a secondary it is that CPU's own periodic timer, counted in its block
/// by `smp` and going no further, since the scheduler is the boot CPU's.
extern "x86-interrupt" fn timer_entry(_frame: idt::InterruptFrame) {
    let chip = irq_chip();
    if smp::on_secondary_tick() {
...
/// A function-call IPI.
extern "x86-interrupt" fn ipi_call_entry(_frame: idt::InterruptFrame) {
    smp::on_ipi(smp::IPI_CALL);
    irq_chip().eoi(IrqNumber(u32::from(IPI_CALL_VECTOR)));
}

/// A reschedule IPI.
extern "x86-interrupt" fn ipi_reschedule_entry(_frame: idt::InterruptFrame) {
    smp::on_ipi(smp::IPI_RESCHEDULE);
