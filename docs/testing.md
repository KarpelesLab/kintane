...
- `udp_peer` sends `kintane-udp-probe` to the forwarded port four times a second, each followed
  by `kintane-tcp-port <tcp>` and by `kintane-udp-fragmented 1 <64 bytes>`, and answers every
  `kintane-udp-echo <n>` the guest sends back with `kintane-udp-ack <n>`. The 64 bytes are
  `i ^ 0x5a`, which the guest checks one by one once it has the datagram whole.
