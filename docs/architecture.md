...
`NR_CPUS` and the port's `HasSmp::MAX_CPUS`, and each one it starts runs an idle loop.
There is still one scheduler, and it is the boot CPU's.
...
`0xF1` (reschedule), addressed by APIC ID, which `init_cpu` returned on the target.
`arch::smp::send(cpu, IPI_CALL | IPI_RESCHEDULE)` has the same names and meaning on both
ports. It raises one IPI and returns without waiting, from any CPU, which is the primitive
a TLB shootdown will build on. The local APIC timer is vector `0xEF` and the spurious
...
**IPIs** on aarch64 are SGIs. SGI 0 runs a function on the target and SGI 1 is a reschedule,
...
- **TLB invalidation is inner-shareable on aarch64** (`tlbi ...is`), which reaches every
  CPU in the system. It is correct for shared kernel mappings. It is not a shootdown
  protocol, and user address spaces will need one. **On x86_64 it is `invlpg`, which
  reaches only the CPU that runs it.** That is harmless today, because a secondary touches
  nothing but its own block, its stack and its local APIC, none of which is ever remapped.
  It stops being harmless the moment a secondary runs kernel code that reads remapped
  memory. A shootdown over `arch::smp::send` has to land first.
