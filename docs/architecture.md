...
- **PCI.** `device::pci` enumerates buses through a `ConfigSpace` the platform provides,
  ...
  - **PCI interrupts: MSI-X, then MSI, then the line.** A function can interrupt three
    ...
    - *Claims.* A vector is claimed like a line, through `Probe::claim_msi`. Its specifier's
      ...
      translated by `Registers::for_claim`. That is the one place a table's address is made,
      so moving device memory to a virtual window changes that function and nothing that
      calls it.
