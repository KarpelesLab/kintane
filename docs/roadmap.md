...
| 2 — Core kernel | **every item landed**; the stress audit is judged in slices rather than wall-clock windows, and `kbuild soak` runs it unattended; **a two-hour soak at 8 CPUs passed 7,200 audits of 7,200** while five branches shared the host. **The 24-hour run has now been attempted, and it failed** — at 14 h 13 m (51,182 s) with `a workload did not reach a checkpoint: ping`, the guest exiting 35 where 33 was expected, after 40,766 clean audits. `ping` is the channel round trip that moves a handle there and back; the audit's two arms tell a workload spinning without checkpointing from one that is never scheduled, and this was the second: marked running, blocked past the 3 s bound against a 50 ms legitimate maximum, with fewer than two timer interrupts finding it on a CPU, while every other counter kept advancing. A stalled host is ruled out from the trail's own clock: the guest tracked wall-clock to 0.11%, and the largest gap between consecutive samples in the whole run was 6 s, once, fourteen hours before the failure. **The cause is now known, and it is not a lost wake.** `ping` exhausted its handle table: every round trip transfers the event handle out and back, one open/close cycle on one slot, and a slot retires after `MAX_GENERATION` = 2²⁰−1 = 1,048,575 cycles. Slot 0 holds the permanent endpoint, and a slot serves `MAX_GENERATION` − 1 cycles — the close whose *next* generation would reach the maximum is the one that retires it — so 255 × 1,048,574 = **267,386,370** round trips exhaust the table; the last heartbeat before the failure recorded `ipc` at **267,383,146**, short by 3,224 — under a second at the observed rate, a match to 99.9988%. Falsified by shrinking `MAX_GENERATION` to 128, which reproduces the identical line in **17 seconds**. The refused receive was then masked: `receive` checks for room before dequeuing, so `ping` looped on the error and never returned to its park check, and `fail()` only records — the recorded *"a receive was refused"* is read inside `audit()`, which runs after the park timeout that killed the run. The message named the symptom and discarded the cause. The earlier guess that a lost cross-CPU wake was to blame came from a stale expected-result: re-run, that mutation names `net` at 1 s, not `ping` at 3 s, and `ping` can never be a lost wake's victim because an empty or full endpoint is a yield, not a block. **The table has since been resized and a day now fits.** The 32-bit handle splits between index and generation, and the index was the half going unused: the largest `HandleTable` in the tree is the ipc workload's 256 slots, every other eight or fewer, so twelve index bits addressed 4,096 slots that do not exist while the generation ran out beneath them. Ten bits still address 1,024 — four times the largest table here — and give each slot 4,194,302 cycles, a ceiling of **1,069,547,010** round trips: **66.2 hours at 4,485/s, a 2.76× margin on a day** (2.37× at the 5,224/s the failing run actually averaged). It costs no memory, since tables are sized by their own `N` rather than by the encoding's maximum; what it costs is that a table wider than 1,024 is now a compile error. The sizing is no longer a phrase but arithmetic the compiler checks: a test counts the cycles a slot serves and fails if the constant is off by one, and `ipc.rs` asserts its round-trip count covers a day at the measured rate. The audit now reports the failure a workload **recorded** rather than the park timeout that followed it, so a run that exhausts the table says *"a receive was refused"* instead of naming the symptom. **A 24-hour run has not yet been made against this; the arithmetic says it should pass, which is not the same as having passed.** |
...
| 4 — Configurability, scaling down | riscv32 (with and without atomics), ARMv7-M at 56 KiB of RAM, `mm::flat`, modules, the full config language, random configs, size budgets. The random sweep now **boots what it builds**: sixteen of the twenty-one boundary configurations build and all sixteen boot, against seven building and none booted a round earlier, once four defects it had found were fixed — three in code and one in the configuration language, where a `preempt.rs` assert was the requirement and nothing stated it where the generator reads. Booting then found three more that building alone could not, sharing one shape: `default … if X` states a requirement `range` does not enforce, so any preset or sample naming the symbol escapes it. It **judges what it boots** as well: a sample built to die is held to the four conditions CI requires — something decoded, the crash's own frame, its caller, and a `crash.rs:` line — so a guest that merely dies does not pass. One property of the instrument belongs with any claim about its coverage: the generator runs two passes and a choice's default member survives only if drawn twice, so P(default) = (1/k)² rather than 1/k, and the configurations it reaches are systematically skewed away from defaults. Real hardware and a thousand random configs remain |
| 5 — Driver isolation | **exit criterion met** on x86_64, and past it: the same virtio-blk core runs in the kernel and in a ring-3 domain, its interrupt delivered as a message, its DMA confined by VT-d with remapped interrupts and queued invalidation; on `x86_64-isolated-smp` the client, the interrupt and the domain each run on a different CPU; a faulting domain dies alone and restarts; the cost is measured. AMD-Vi and per-domain quotas remain; on aarch64 the SMMUv3 is found and reported — its stream-id width, its granules and its output size — but nothing on that port is behind it, because the machine maps stream ids only from the PCIe root complex while this port's disk sits in a memory-mapped virtio slot, so confinement there waits on a device being on PCIe at all: the bridge is now found from the device tree and the bus behind it walked, at the first moment the device mapping reaches a window the boot tables cannot cover, and a disk is attached to it; what remains is binding one, and the obstacle has moved again — this time to something that sat in front of everything previously ruled out. **Every base address register read zero.** The function enumerates, matches and sizes correctly, but `virt` booted with `-kernel` runs no firmware and nothing had assigned the registers, so a driver bound to it would have mapped address zero; `pci.rs` had said as much under *What is not here*. They are now placed at natural alignment from an arena reserved **during discovery** — the walk that learns what a register needs cannot run until the address space exists, because configuration space sits a quarter of a terabyte above the boot tables, so room is taken from what the tree says the bridge forwards and filled afterwards. Memory decode is enabled with the placement, since an assigned register whose decoder is off answers nothing. The endpoint now answers `VIRTIO_F_VERSION_1` at the address it was given, which distinguishes a mapped window from both an unmapped one (all ones) and a silent one (zero). What is left is smaller but partly **unbuilt rather than unwired**: `device::tree` does not interpret an `interrupt-map` nexus at all — `tree.rs` returns `Error::InterruptNexus`, "not interpreted yet" — so the translation from a function's pin to a GIC input does not exist, and `MAX_DISKS` should become a configuration symbol before it becomes a three, since a third slot costs every preset `.bss` for a slot only `aarch64-pcie` can fill. A third disk image now exists and each disk's bytes are keyed by what it carries rather than the slot it sits in, so a third slot no longer routes to the second disk's image; `MAX_DISKS` stays 2 until the binding lands, since a third slot costs every preset a `VirtioBlk` in `.bss` for a slot only `aarch64-pcie` could fill |
...
### The twenty-first round of landings

Four briefs, and every premise they were given was overturned by the brief testing it.

- **The soak's ceiling, raised from arithmetic rather than hope.** A handle is one `u32` split
  between index and generation, and the index was the half going unused: the largest `HandleTable`
  in the tree is the ipc stress workload's 256 slots, every other eight or fewer, so twelve index
  bits addressed 4,096 slots that do not exist while the generation ran out beneath them. Ten bits
  still address 1,024 and give each slot 4,194,302 cycles — a ceiling of 1,069,547,010 round trips,
  66.2 hours at the rate this kernel sustains, a 2.76× margin on a day. It costs no memory. The
  sizing is no longer a phrase: a test counts the cycles a slot serves and fails if the constant is
  off by one, and `ipc.rs` asserts its round-trip count covers a day at the measured rate. **The
  figure this project published for the old ceiling was itself wrong by 255** — a slot serves
  `MAX_GENERATION` − 1 cycles, since the close whose next generation would reach the maximum is the
  one that retires it.
- **The audit now reports the cause it already knew.** A workload that recorded a failure and then
  stopped checkpointing was reported by the park timeout that followed, not by what it recorded —
  `audit()` checked the recorded failure first but was only reached when `park_everything` returned,
  and `park_everything` ends the run itself. The ordering at the call site was the whole bug.
- **A boot-stack overflow that no single symbol causes.** `INKERNEL_TESTS=y` with
  `BOOT_ARGS_CHECK=n` overflows `armv7m-tiny`'s 14 KiB boot stack; each alone is comfortable at
  9,592 and 10,120 bytes, together they reach 14,896 — 4,752 bytes of interaction, not a sum, and a
  MemManage fault in the guard page rather than an artefact of the painting heuristic. The stack
  goes to 16 KiB, not 20, because the point is to be able to **report** such a depth: at 20 the
  guard passes at 72% and says nothing. **Why the same path runs 4,776 bytes deeper was not
  established, and is recorded as unexplained rather than guessed at.**
- **Two floors said conditionally, where they were stated flatly.** `THREAD_STACK_KIB` at 4 on
  x86_64 resolved cleanly and then died at *link* — the port asserts 8 KiB where ARMv7-M asserts 1,
  because there the guard is a quarter of the slot rather than a page. `QEMU_MEMORY_MB`'s floor
  belongs to the firmware and was measured across six values: EFI builds fail at 52 and pass at 56,
  so 56 is the floor, not a round 64. Below it a guest prints nothing and exits 0, which reads as a
  hang — why an 8 MiB sample looked like one. The power-of-two requirement `hal::StackArray` needs
  is stated as **inexpressible**: a range is a floor and a ceiling, and the linker is the only
  instrument.
- **Base address registers no firmware assigned.** See Phase 5.

Two sweeps of the record were needed and are worth recording as a habit: a note saying a conflict
was "not fixed here" survived the round that fixed it, and a claim that a legacy interrupt pin
"routes to a GIC SPI the existing driver already handles" described the device tree while saying
nothing about whether the kernel reads it — it does not. Corrections had been appended as new
sections while the contradicted passages stood, so the document asserted both.

What this round did not do: the 24-hour soak has not been re-run against the larger table — the
arithmetic says it should pass, which is not the same as having passed — and the disk on the
aarch64 PCIe bus is still not bound.

### The twentieth round of landings

Four briefs. Every premise the round began with was overturned by the brief testing it.

- **Why the 24-hour soak failed, found and falsified in seventeen seconds.** `ping` exhausted its
  handle table: each round trip transfers the event handle out and back, a slot retires after
  1,048,575 generations — a slot serving one fewer than that, since the close whose next generation would reach the maximum retires it — and with slot 0 permanently held, 255 × 1,048,574 = 267,386,370 round
  trips exhaust it — against `ipc` = 267,383,146 at the last heartbeat, short by 3,224. Shrinking
  `MAX_GENERATION` to 128 reproduces the identical line in 17 s rather than 14 h. The failure
  message had been discarding its own cause: `receive` checks for room before dequeuing, so `ping`
  looped on the error and never reached its park check, and the recorded *"a receive was refused"*
  is read inside `audit()`, which runs after the park timeout that ended the run. The audit now
  reports **why** a workload missed its deadline, not only which one did.
- **A lead that was wrong, and the stale record behind it.** `docs/testing.md` had recorded the
  no-IPI mutation as naming `ping` at 3 s. Re-run, it names `net` at 1 s — the row predates the
  `net`, `tcp`, `block` and `fs` workloads — and `ping` can never be a lost wake's victim at all,
  because an empty or full endpoint is a yield rather than a block. **A stale expected-result is
  worse than none**: this one sent a fourteen-hour lead after the wrong mechanism.
- **Two requirements that lived only in a `default`, said as ranges.** The configuration language
  was not missing a mechanism — `range … if` already existed; what was missing was saying the
  requirement in it. Enumerating all 87 `config` blocks rather than sampling found `QEMU_CPUS`,
  which no sweep had reported: at `QEMU_CPUS=1` a MADT walk that stops after its first entry passes
  regardless, so the check stops being able to fail. A floor for `BOOT_STACK_KIB` was **refused**,
  because the configuration it would forbid demonstrably passes at 70% of its stack.
- **The sweep's random half judges now.** Every crash sample was being skipped; all six at seed 5000
  are now judged, against the four conditions CI already requires — something decoded, the crash's
  own frame, its caller, and a `crash.rs:` line, the last two being what stop it from being
  vacuous. The bias that made nearly every sample a crash was measured and the old explanation
  corrected: `CRASH_NONE` *is* proposed, about a third of the time, but the generator runs two
  passes and a default survives only if drawn twice, so P(default) = (1/k)² rather than 1/k — 10.0%
  measured over 180 seeds. That holds for **every** choice in the language, so the sweep has been
  reaching configurations systematically skewed away from defaults.
- **Every driver is compiled in every configuration, measured and kept.** The waste is real —
  10,447 ms of 225,695 ms of rustc, 4.5% of a sweep — but gating it costs more than it saves:
  host tests are selected from *planned* units, so gating one driver took a host suite from 41
  units to 40 **while the build still exited 0**. The cheap-looking change would have silently
  removed the coverage that keeps the Arm drivers tested at all.

Two briefs converged on one defect from opposite directions: no single symbol causes the
`armv7m-tiny` boot-stack overflow, and the decode names where it happens —
`selftest::heap::exhaust_and_return`. What this round did not do: the 24-hour soak still has not
passed, and cannot until the handle table is sized for the rate the kernel now achieves; the disk
on the aarch64 PCIe bus is still not bound.
