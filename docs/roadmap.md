...
| 2 — Core kernel | **every item landed**; the stress audit is judged in slices rather than wall-clock windows, and `kbuild soak` runs it unattended; **a two-hour soak at 8 CPUs passed 7,200 audits of 7,200** while five branches shared the host. **The 24-hour run has now been attempted, and it failed** — at 14 h 13 m (51,182 s) with `a workload did not reach a checkpoint: ping`, the guest exiting 35 where 33 was expected, after 40,766 clean audits. `ping` is the channel round trip that moves a handle there and back; the audit's two arms tell a workload spinning without checkpointing from one that is never scheduled, and this was the second: marked running, blocked past the 3 s bound against a 50 ms legitimate maximum, with fewer than two timer interrupts finding it on a CPU, while every other counter kept advancing. A stalled host is ruled out from the trail's own clock: the guest tracked wall-clock to 0.11%, and the largest gap between consecutive samples in the whole run was 6 s, once, fourteen hours before the failure. **The cause is now known, and it is not a lost wake.** `ping` exhausted its handle table: every round trip transfers the event handle out and back, one open/close cycle on one slot, and a slot retires after `MAX_GENERATION` = 2²⁰−1 = 1,048,575 cycles. Slot 0 holds the permanent endpoint, so 255 slots × 1,048,575 = **267,386,625** round trips exhaust the table; the last heartbeat before the failure recorded `ipc` at **267,383,146**, short by 3,479 — under a second at the observed rate, a match to 99.9987%. Falsified by shrinking `MAX_GENERATION` to 128, which reproduces the identical line in **17 seconds**. The refused receive was then masked: `receive` checks for room before dequeuing, so `ping` looped on the error and never returned to its park check, and `fail()` only records — the recorded *"a receive was refused"* is read inside `audit()`, which runs after the park timeout that killed the run. The message named the symptom and discarded the cause. The earlier guess that a lost cross-CPU wake was to blame came from a stale expected-result: re-run, that mutation names `net` at 1 s, not `ping` at 3 s, and `ping` can never be a lost wake's victim because an empty or full endpoint is a yield, not a block. **A 24-hour soak cannot pass until the table is sized for the rate now achieved**: 267,386,625 round trips over 86,400 s is a ceiling of 3,095/s, and this kernel sustains a median 4,485/s — 45% faster — so a full day needs room for about 387 million |
...
- **Why the 24-hour soak failed, found and falsified in seventeen seconds.** `ping` exhausted its
  ...
  1,048,575 generations, and with slot 0 permanently held, 255 × 1,048,575 = 267,386,625 round
  trips exhaust it — against `ipc` = 267,383,146 at the last heartbeat, short by 3,479. Shrinking
