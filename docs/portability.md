...
For builds that cannot afford a vtable in the interrupt path, the configuration may
leave exactly one implementation in the image, and then the interrupt path calls it
directly.

**As planned**, this was a type alias `kbuild` would generate from a symbol such as
`CONFIG_IRQCHIP=nvic`. **As built** it is not an alias, because of the layering rule: the
interrupt path lives in `arch`, and `arch` may not name a driver's type, which lives in
the device layer above it. An alias `arch` could see would have to name something `arch`
is forbidden to depend on. So the mechanism is a generic function plus a linker symbol:

- `arch::irq::dispatch_with<C: IrqChip + ?Sized>(chip: &C)` holds the whole dispatch
  loop — claim, identify, handle, acknowledge — once, in `arch`.
- In an image with several drivers, the IRQ vector reads the installed
  `&'static dyn IrqChip` and calls `dispatch_with` with `C = dyn IrqChip`: vtable calls.
- With `IRQCHIP_STATIC`, the platform provider that bound the one driver instantiates
  `dispatch_with` for that driver's concrete type and exports it as
  `kintane_irq_dispatch`. The vector calls that symbol, which the linker resolves. Every
  call inside the loop is direct.

One source, no duplicate implementation. A function pointer would have crossed the layer
boundary too, but it would have left one indirect call per interrupt, which is most of
what the vtable cost.

`IRQCHIP_STATIC` is refused while two drivers are enabled, because then only the running
machine knows which one answers. On aarch64 it follows from the driver selection: the
default image keeps `GIC_V2` and `GIC_V3` and stays dynamic, and `aarch64-virt-gicv3`
turns `GIC_V2` off and becomes static.

**What it buys**, measured on `aarch64-virt` against `aarch64-virt-gicv3` (same kernel,
`SMP=n`, one GIC driver fewer):

| | dynamic (two drivers) | single-provider (GICv3) |
|---|---|---|
| indirect calls per claimed interrupt | 3: `claim`, `id`, `eoi` | 0 |
| the acknowledgement | a vtable call | one instruction, `msr ICC_EOIR1_EL1`, inlined |
| code in `.text` symbols | 192,560 bytes | 191,076 bytes (−1.5 KiB) |
| boots on `gic-version=2` | yes, same binary | no: fails naming the unbound controller |

The code saving is small and disappears into section alignment in the image total; the
point is the interrupt path, not the size. The table is from the disassembly, not from a
timing, because a timing under QEMU's TCG measures the emulator. The CI step "Interrupt
dispatch is static where it should be" disassembles both images and fails if the static
one has an indirect branch on the path, or if the dynamic one has none, so the check
cannot pass by being unable to see.

**What it costs:** a board-specific image. The `gic-version=2` row is the point of the
dynamic default, and a single-provider image gives it up on purpose.

Ports where there is only ever one controller never needed this. ARMv7-M's NVIC is part
of the architecture, so its interrupt path was a direct call from the first commit, and
it has no `IrqChip` at all. x86 carries both the 8259A and the APIC and stays dynamic.
