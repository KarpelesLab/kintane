...
| `sendmsg`, `recvmsg` | up to four `iovec`s, gathered into one message and scattered from one receive in order; more is `EOPNOTSUPP`. `sendmsg` sends to `msg_name` when it has one, `recvmsg` writes the sender there and its length into `msg_namelen`. Control data is ignored |
