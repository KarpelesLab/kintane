...
//! interrupt-driven receive: the driver is transmit-only there, and says so by never
//! offering an interrupt to wire up.

/// Whether this build receives on interrupt.
pub const RECEIVES: bool = false;

/// Throws bytes away. Nothing on such a machine drains the FIFO, since no handler runs.
pub struct Queue;
