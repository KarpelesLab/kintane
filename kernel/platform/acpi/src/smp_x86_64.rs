...
pub(crate) unsafe fn start_secondaries(c: &dyn EarlyConsole) -> Option<bool> {
    ...
    ok &= user_entry_is_per_cpu(c, started);
...
/// The marker CPU `cpu` installs as its user-entry kernel stack. Distinct per CPU and never
/// a stack: nothing enters ring 0 from ring 3 during bring-up.
const fn marker_for(cpu: usize) -> u64 {
    0x5ec0_0000_0000 + cpu as u64
}

/// Every CPU, asked on itself, installs its own user-entry kernel stack into its own TSS and
/// finds it again through its own `GS`, and has `syscall` enabled where user mode is built.
/// Then every CPU's `rsp0` is read back from the boot CPU: a write that landed in another
/// CPU's TSS leaves some CPU holding the wrong marker. This is what lets a user thread run on
/// any CPU, and what a shared `rsp0` would break: a trap from ring 3 on one CPU landing on
/// the kernel stack of a thread running on another.
fn user_entry_is_per_cpu(c: &dyn EarlyConsole, cpus: usize) -> bool {
    c.write_str("; user entry");
    let want = smp::PROBE_RSP0
        | smp::PROBE_GS_STACK
    ...
    for cpu in 0..cpus {
        let bits = if cpu == 0 {
            Some(smp::probe_user_entry(marker_for(0)))
        } else {
            smp::call(cpu, smp::probe_user_entry, marker_for(cpu))
        };
        if bits.is_none_or(|b| b & want != want) {
            c.write_str(" WRONG(");
            write_usize(c, cpu);
            c.write_str(")");
            ok = false;
    ...
    for cpu in 0..cpus {
        if smp::kernel_stack_of(cpu) != marker_for(cpu) {
            c.write_str(" RSP0 OF CPU ");
            write_usize(c, cpu);
            c.write_str(" IS ANOTHER CPU'S");
            ok = false;
        }
    }
    if ok {
