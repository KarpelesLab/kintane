...
/// Why a blocking socket call must stop waiting, other than its process ending. Nothing yet:
/// when signals exist, a signal to deliver ends the wait here with `EINTR`.
fn interrupted(_slot: usize) -> Option<Failure> {
    None
}
