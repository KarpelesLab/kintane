...
pub fn setup() -> Result<(), &'static str> {
    ...
    // SAFETY: no workload thread exists yet, so the boot thread is the only user.
    let Some(fat) = (unsafe { crate::fs::volume() }) else {
        return Ok(());
    };
    fat.check_cache()
...
pub fn audit() -> Result<(), &'static str> {
    ...
    // SAFETY: the workload thread is parked at a checkpoint, so the auditor is the volume's
    // only user for the length of this call.
    let Some(fat) = (unsafe { crate::fs::volume() }) else {
        return Err("the volume went away");
    };
    fat.check_cache()?;
...
pub extern "C" fn worker(_: usize) -> ! {
    ...
    // SAFETY: this thread is the one the volume's invariant names.
    let Some(fat) = (unsafe { crate::fs::volume() }) else {
    ...
    loop {
        if park_requested() {
            // Between iterations every handle is closed, so there is nothing to hold.
