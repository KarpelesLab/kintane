...
/// Frames for the rings, one request's header and status, and the bounce buffer. Few
/// enough that the check's 32-sector read cannot fit in one request, so the block layer's
/// split is exercised against the real device.
